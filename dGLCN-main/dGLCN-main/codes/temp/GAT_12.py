...
from getData import get_BL_Data,get_data_AD_SF,get_12M_Data
...
class GAT(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, heads):
        ...
        self.conv2 = GATConv(hidden_channels * heads, hidden_channels // 2, heads=1,
...
def train_SF():
    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        logits = model(tensor_x, edge_index)
...
for dataset_index in range(len(dataset)):
    if __name__ == "__main__":
        ...
        for j in range(10):
            x,edge_index,edge_weight,labels, rand_list,rand_listBL = get_12M_Data(j, dataset[dataset_index])
            ...
            Temptest = np.zeros((x.shape[0], 2))
            Tempdec = np.zeros((x.shape[0], 2))
            ...
            for idx_train, idx_test in stratified_kfold.split(x,y1):
                ...
                model = GAT(input, 50,output,8).to(device)
            ...
            Temptest[rand_listBL] = Ltest[0:len(rand_listBL)]
            Tempdec[rand_listBL] = Ldec[0:len(rand_listBL)]

            Ltest[rand_listBL] = Temptest[rand_listBL]
            Ldec[rand_listBL] = Tempdec[rand_listBL]
        ...
        file_path = 'R1_D1101_3.12.Mat__PDvsSW7_GAT_12m.mat'
