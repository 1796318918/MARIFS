...
from utils import feature_normalized, create_graph_from_embedding, sample_mask, preprocess_adj, preprocess_features,sim_matrix_euclidean_distance
import numpy as np
...
from getData import get_BL_Data
...
def train_SF():
    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        logits = model(tensor_x, edge_index,edge_weight)
...
def test_SF(mask):
    ...
    out = model(tensor_x, edge_index,edge_weight)
    mask_logits = out[mask]
    probabilities = F.softmax(out[mask], dim=1)
    ...
    return accuracy, sensitivity, specificity, auc, f1, Prec, Uar,label2,probabilities
...
if __name__ == "__main__":
    ...
    feature_file = './data/PPMI_2classes_' + str(args.index) + '.mat'
    data_feature = sio.loadmat(feature_file)

    x=data_feature['features']
    features1 = torch.tensor(x, dtype=torch.float32)
    adj = sim_matrix_euclidean_distance(features1, 0.002)
    edge_index, edge_weight = dense_to_sparse(adj)

    labels = data_feature['sampleLabel'].flatten()
    ...
    train_mask = np.array(data_feature['train_mask'], dtype=np.bool_).flatten()
    val_mask = np.array(data_feature['val_mask'], dtype=np.bool_).flatten()
    test_mask =np.array(data_feature['test_mask'], dtype=np.bool_).flatten()

    tensor_x = torch.from_numpy(x).to(torch.float32).to(device)

    model = GCN(input, 50, output).to(device)
    ...
    maxAuc_Ltest=maxAuc_Ltest[:,1]
    maxAuc_Ldec = maxAuc_Ldec.cpu().detach().numpy()
    maxAuc_Ldec=maxAuc_Ldec[:,1]

    save_adr = './result/PPMI_2classes_result_' + str(args.index) + '.mat'
    sio.savemat(save_adr, {'test_accs': test_accs, 'test_sen': test_sen, 'test_spe': test_spe, 'test_auc': test_auc,
                           'test_f1': test_f1, 'test_prec': test_prec, 'test_uar': test_uar, 'maxAuc_Ltest': maxAuc_Ltest,
                           'maxAuc_Ldec': maxAuc_Ldec})
...
#         file_path = 'R1_D1111_3.12.Mat__PDvsSW5_GCN_baseline.mat'
#
#         # 使用 savemat 保存.mat文件
