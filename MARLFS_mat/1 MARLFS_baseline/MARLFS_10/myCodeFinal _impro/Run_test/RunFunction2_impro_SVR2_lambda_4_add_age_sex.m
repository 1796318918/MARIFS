...
%                             sample_lack_index(index_id+1,2)=0; 
                        ...
                        for i=1:num_M
                            ...
                            XData_M{i}=my_convert2Sparse_impro2(XData_M{i});
                        end
                        
                        temp_trainData=[];
                        temp_testData=[];
                        for i=1:num_M
                            temp_trainData = [temp_trainData,XData_M{i}(1:size(trainData,1),:)];
                            temp_testData = [temp_testData,XData_M{i}(size(trainData,1)+1:end,:)];
                        end
                        
                        save_path='D:\other_people_l\DX\competing_method\improve_3\Incomplete\3.MARLFS\dGLCN-main\dGLCN-main\codes\data\';
                        index_temp=1:size(temp_trainData,2)+1;
                        data_temp=[temp_trainData,trainFlag];
                        data_pd=[index_temp;full(data_temp)];
                        csvwrite([save_path,'PPMI_2classes_',num2str(ii),'.csv'], data_pd); 
                        
                        %python脚本调用，跨平台编程 hzw 2024.4.14
                        system(['powershell -File "D:\other_people_l\DX\competing_method\improve_3\Incomplete\3.MARLFS\run_python_script.ps1" -Index ',num2str(ii)]);
                        
                        pause(5);
                        load(['D:\other_people_l\DX\competing_method\improve_3\Incomplete\3.MARLFS\dGLCN-main\dGLCN-main\codes\result\PPMI_2classes_result_',num2str(ii),'.mat']); % 读取txt文件数据
                        
                        %hzw 2021.6.28
                        normW=state1;
                        SelectFeaIdx{kk,ii} = find(normW~=0); % SelectFeaIdx: 最相关的特征下标

                        newTrainData = temp_trainData(:,SelectFeaIdx{kk,ii}); % 选取最相关的特征维度
                        newTestData = temp_testData(:,SelectFeaIdx{kk,ii});
                        %将权重与脑区对应起来
