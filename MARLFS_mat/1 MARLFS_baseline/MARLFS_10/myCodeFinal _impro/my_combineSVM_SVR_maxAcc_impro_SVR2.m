function [svmMaxAcc,accSen,accSpec,accPrec,accFscore,maxAuc,maxAcc,minArmse,maxBcc,minBrmse,maxCcc,minCrmse,maxDcc,minDrmse,Rs,Ra,Rb,Rc,Rd]=...
    my_combineSVM_SVR_maxAcc_impro_SVR2(trainData,trainFlag,trainScoresA,trainScoresB,trainScoresC,trainScoresD,...
    testData,testFlag,testScoresA,testScoresB,testScoresC,testScoresD,parc,parg)

    % 2016.04.21 修改：
    % 增加了最大AUC，对应的 Ldec 和 Ltest 
...
%         cmdSVR1 = ['-t 2 -s 3',' -c ' num2str(2^parc(i)),' -g ' num2str(10^parg(j))]; % 默认类型(RBF核)
        cmdSVR1 = ['-t 3 -s 3',' -c ' num2str(2^parc(i)),' -g ' num2str(10^parg(j))]; % sigmoid核
...
[svmMaxAcc,accSen,accSpec,accPrec,accAuc,accFscore,accLdec,accLtest,accTP,accFP,accBest_X,accBest_Y,maxAuc,aucLdec,aucLtest,aucTP,aucFP,aucBest_X,aucBest_Y]=...
    my_findBestAcc_SVM(SVMacc,SVMsen,SVMspec,SVMprec,SVMauc,SVMfscore,SVMldec,SVMltest,SVMtp,SVMfp);
...
Rc.maxAcc_Ccc=maxCcc;
Rc.maxAcc_rmse=CccRmse;
Rc.maxAcc_Ldex=CccDec;
Rc.maxAcc_Ltest=CccTest;
Rc.maxAcc_X=CccBest_X;
Rc.maxAcc_Y=CccBest_Y;
 
Rd.maxAcc_Bcc=maxDcc;
Rd.maxBcc_rmse=DccRmse;
Rd.maxBcc_Ldex=DccDec;
Rd.maxBcc_Ltest=DccTest;
Rd.maxBcc_X=DccBest_X;
Rd.maxBcc_Y=DccBest_Y;

Rs.maxSvmAcc_auc=accAuc;
Rs.maxSvmAcc_Ldec=accLdec;
Rs.maxSvmAcc_Ltest=accLtest;
Rs.maxSvmAcc_tp=accTP;
Rs.maxSvmAcc_fp=accFP;
Rs.maxSvmAcc_X=accBest_X;
Rs.maxSvmAcc_Y=accBest_Y;
