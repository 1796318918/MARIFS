...
tp = sum((yp == 1) .* (ylabel==1)); % 检测到的正样本数目,score>0 且标签确实为正的
fp = length(find(yp == 1))-tp;

sensitivity = tp/sum(ylabel==1); % R(召唤率)：检索到的正样本/正样本lable个数(不管有没有检测到)
sensitivity(find(isinf(sensitivity))) = eps; % 若本来就没有正样本，那么会出现∞
sensitivity(find(isnan(sensitivity))) = eps; 

tn = sum((yp == -1) .* (ylabel==-1)); % 检测到的负样本数目,score<0 且标签确实为负的
...
Precision = tp/sum(yp == 1); %  精度：检索正确的正样本/检索到的正样本
